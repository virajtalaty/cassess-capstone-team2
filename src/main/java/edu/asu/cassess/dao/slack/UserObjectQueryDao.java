...
public class UserObjectQueryDao implements IUserObjectQueryDao {
        ...
        public <T> Object getUserByEmail(String course, String email) throws DataAccessException {
            Query query = getEntityManager().createNativeQuery("SELECT DISTINCT * FROM slack_user WHERE course = ?1 AND email = ?2", UserObject.class);
    ...
    public <T> Object getUserByDisplayName(String course, String display_name) throws DataAccessException {
        Query query = getEntityManager().createNativeQuery("SELECT DISTINCT * FROM slack_user WHERE course = ?1 AND display_name = ?2", UserObject.class);
        query.setParameter(1, course);
    ...
    public <T> Object deleteUserByEmail(String course, String email) throws DataAccessException {
        Query query = getEntityManager().createNativeQuery("DELETE FROM slack_user WHERE course = ?1 AND email = ?2");
        query.setParameter(1, course);
        query.setParameter(2, email);
        query.executeUpdate();
    ...
    public <T> List<UserObject> getUsersByEmail(String email) throws DataAccessException {
        Query query = getEntityManager().createNativeQuery("SELECT DISTINCT * FROM slack_user WHERE email = ?1", UserObject.class);
    ...
    public <T> List<UserObject> getUsersByDisplayName(String display_name) throws DataAccessException {
        Query query = getEntityManager().createNativeQuery("SELECT DISTINCT * FROM slack_user WHERE display_name = ?1", UserObject.class);
